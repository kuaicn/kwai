...
function buildApiO0(method: string, originalHeaders?: Record<string, string>): string {
...
class ProxyAxios {
  ...
  private async request<T = any>(
    ...
    url: string,
    data?: any,
    config?: AxiosRequestConfig & { originalHeaders?: Record<string, string> },
  ) {
    const { originalHeaders, ...restConfig } = config || {}
    return this.client.post<T>('/api/v1/request', data, {
      ...restConfig,
      headers: {
        ...restConfig.headers,
        'api-o0': buildApiO0(method, originalHeaders),
        'api-u': url,
      },
    })
  }

  get<T = any>(url: string, config?: AxiosRequestConfig & { originalHeaders?: Record<string, string> }) {
    return this.request<T>('GET', url, undefined, config)
  }

  post<T = any>(url: string, data?: any, config?: AxiosRequestConfig & { originalHeaders?: Record<string, string> }) {
    return this.request<T>('POST', url, data, config)
  }

  put<T = any>(url: string, data?: any, config?: AxiosRequestConfig & { originalHeaders?: Record<string, string> }) {
    return this.request<T>('PUT', url, data, config)
  }

  patch<T = any>(url: string, data?: any, config?: AxiosRequestConfig & { originalHeaders?: Record<string, string> }) {
    return this.request<T>('PATCH', url, data, config)
  }

  delete<T = any>(url: string, config?: AxiosRequestConfig & { originalHeaders?: Record<string, string> }) {
